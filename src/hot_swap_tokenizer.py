...
def hot_swap_tokenizer(model_name: str, tokenizer_path: str):
...
if __name__ == "__main__":
    ...
    tokenizer_path = DATA_DIR / f"{tokenizer}.json"
    
    model, tokenizer = hot_swap_tokenizer(model_name, tokenizer_path)

    # Save model with new tokenizer
    output_path = DATA_DIR / f"{model_name.split('/')[-1]}-{tokenizer}_tokenizer"
